        ...
        FusionFix::onInitEventAsync() += []()
            {
                ...
                // Mouse look sensitivity slider and multipliers
                {
                    ...
                    bool firstperson_dll = false;
                    bool xlive_dll = false;
                    ModuleList dlls;
                    dlls.Enumerate(ModuleList::SearchLocation::LocalOnly);
                    for (auto& e : dlls.m_moduleList)
                    {
                        if (iequals(std::get<std::wstring>(e), L"firstperson") && !IsModuleUAL(std::get<HMODULE>(e)))
                            firstperson_dll = true;
                        if (iequals(std::get<std::wstring>(e), L"xlivelessaddon") && !IsModuleUAL(std::get<HMODULE>(e)))
                            xlive_dll = true;
                    }
                    if (!firstperson_dll && !xlive_dll)
                    {
